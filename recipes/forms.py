    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "例: 肉じゃが"}
            ),
...
class QuickRecipeForm(RecipeForm):
    """材料・手順を省略して献立を登録するフォーム。"""

    guess_details = forms.BooleanField(
        label="材料・手順をAIで推測",
        initial=True,
        required=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["servings"].initial = 4
...
class IngredientForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "group": forms.TextInput(
                attrs={
                    "class": "form-control form-control-sm group-input",
                    "placeholder": "A / タレ…",
                }
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.quantity is not None:
            self.initial["quantity"] = self._decimal_to_fraction_str(
    ...
    def clean(self):
        ...
        if qty is not None and amount_text:
            raise forms.ValidationError(
                "「数量+単位」と「テキスト分量」は同時に入力できません。どちらか一方を入力してください。"
            )
...
StepFormSet = inlineformset_factory(
    ...
    widgets={
        ...
        "description": forms.Textarea(
            attrs={"class": "form-control", "rows": 2, "placeholder": "手順を入力"}
        ),
    },
    labels={
        "description": "手順",

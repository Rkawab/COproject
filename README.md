...
- **分数入力対応** - 数量欄に `1/2`・`1/8`・`1 1/2` 等の分数テキストを直接入力可能。DB には小数（0.125 等）で保存し、表示時は ⅛・¼・½ 等のUnicode分数に自動変換

### 📷 手書きレシピ画像読み取り
- **AIによるレシピ画像OCR（OpenAI `gpt-4.1-mini`）**
  - 手書きノートやレシピの写真をアップロードすると、AIが内容を読み取りフォームに自動入力
  - 分数（1/2、1/8 等）をそのまま認識しフォームに分数表示で自動入力
...
- **WSGIサーバ**: Gunicorn
- **環境管理**: python-dotenv
- **AI（栄養価推定・画像OCR・材料パース）**: OpenAI API `gpt-4.1-mini`
